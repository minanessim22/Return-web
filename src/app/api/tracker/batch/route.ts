...
import { insertLocationHistoryBatch, isTrackerRegistered } from '@/lib/server/tracker-db';
import type { LocationHistoryRow } from '@/lib/server/tracker-db';
...
export async function POST(request: Request) {
  ...
  // Check geofences with the latest point in the batch
  if (emitList.length > 0) {
    const lastPt = emitList[emitList.length - 1];
    checkGeofences(deviceId, lastPt.lat, lastPt.lon).catch(() => {});
  }
