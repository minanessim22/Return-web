 ...
 * Reusable panel for dashboards and the /tracking/history page.
 */

import React, { useEffect, useState, useCallback } from 'react';
import dynamic from 'next/dynamic';
import Link from 'next/link';
import type { CircleOverlay, MarkerPoint } from '@/components/Map';
import {
  MapPin,
  BarChart3,
  Shield,
...
export function TrackingHistoryPanel({ deviceId, deviceLabel, embedded = false }: TrackingHistoryPanelProps) {
  ...
  const loadStats = useCallback(async (date: string) => {
    ...
    try {
      const r = await fetch(`/api/tracker/stats?device_id=${encodeURIComponent(deviceId)}&date=${date}`);
  ...
  const loadTrail = useCallback(async (date: string) => {
    ...
    try {
      const r = await fetch(`/api/tracker/history?device_id=${encodeURIComponent(deviceId)}&from=${date}T00:00:00Z&to=${date}T23:59:59Z&limit=2000`);
  ...
  const loadGeo = useCallback(async () => {
    ...
    setGeoLoading(true);
    try {
      const r = await fetch(`/api/tracker/geofences?device_id=${encodeURIComponent(deviceId)}`);
      const d = await r.json();
  ...
  const loadLatest = useCallback(async () => {
    if (!deviceId) return;
    try {
      const r = await fetch(`/api/tracker/latest?device_id=${encodeURIComponent(deviceId)}`);
      const d = await r.json();
      setLatest(d.device ?? null);
    } catch {
      setLatest(null);
    }
  }, [deviceId]);
  ...
  const createFence = async () => {
    if (!deviceId) return notify('Please select a device first', false);
  ...
  const liveMarker: MarkerPoint | null = latest && latest.lat !== null && latest.lon !== null
    ? {
    ...
    : null;
  const lastTrailPoint = trail.length > 0 ? [trail[trail.length - 1].lat, trail[trail.length - 1].lon] as [number, number] : null;
  const fallbackMarker: MarkerPoint | null = !liveMarker && lastTrailPoint
    ? { position: lastTrailPoint, label: 'Last location' }
    : null;
  const trailMarkers = liveMarker ? [liveMarker] : fallbackMarker ? [fallbackMarker] : [];
  const mapCenter: [number, number] = (liveMarker?.position || lastTrailPoint) ?? [30.0444, 31.2357];
  const canShowTrailMap = trailMarkers.length > 0 || trail.length >= 2;
  ...
  const headerClass = embedded
    ...
    : 'bg-white/80 backdrop-blur-xl border-b border-gray-100 sticky top-0 z-40';
  const containerClass = embedded
    ? 'rounded-3xl border border-gray-100 bg-white/95 shadow-xl overflow-hidden'
    : 'min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50/30';
  const innerClass = embedded ? 'px-4 sm:px-6 py-4' : 'max-w-5xl mx-auto px-4 sm:px-6 py-4';
  const mainClass = embedded ? 'px-4 sm:px-6 py-6 pb-10' : 'max-w-5xl mx-auto px-4 sm:px-6 py-6 pb-20';
  const deviceName = deviceLabel || deviceId;
          ...
          <div className="flex items-center gap-3 mb-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-gray-400 font-medium mt-0.5">
                Device: <span className="text-[#014CB3] font-bold">{deviceName}</span>
              </p>
            </div>
            ...
            <div className="rounded-2xl overflow-hidden border border-gray-200 shadow-lg" style={{ height: showCreate ? '320px' : '260px' }}>
              <DynamicMap
                ...
                markers={[
                  ...
                  ...(fenceCenter ? [{ position: fenceCenter, label: fenceName || 'New geofence' }] : []),
                ]}
                zoom={13}
                scrollWheelZoom={true}
                showControls={true}
